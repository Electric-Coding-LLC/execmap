# Changelog

All notable shipped changes to `@electric_coding/execmap` are recorded here.

This file tracks published package history. Plans under `plans/*` track
execution, not shipped versions.

## 0.4.0 - 2026-04-20

### Changed

- ship 0.4 lean authoring flow (#9)

## 0.3.0 - 2026-04-20

### Changed

- Fix issue with skill
- add plan lifecycle commands (#8)

## 0.2.0 - 2026-04-20

### Changed

- add execmap done command (#6)
- simplify release flow (#7)

## 0.1.2 - 2026-04-19

### Fixed

- Preserve dotted release names in scaffolded initiative folders.

## 0.1.1 - 2026-04-19

### Changed

- Scope the published package contents for installed users.
- Add the first package release helper flow for cutting tagged releases.

## 0.1.0 - 2026-04-19

### Added

- Initial public release of the Bun-first `execmap` CLI.
- Ship the `init`, `next`, and `check` workflow for bounded execution maps.



